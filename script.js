...
function playBirdSound(birdKey) {
    const sound = birdSounds[birdKey];
    if (sound) {
        const soundElement = document.createElement('div');
        soundElement.className = 'bird-sound';
        ...
        soundElement.style.fontSize = '3em';
        soundElement.style.pointerEvents = 'none';
        soundElement.style.zIndex = '9999';
        soundElement.style.animation = 'popSound 0.5s ease-out forwards';
...
function createConfetti() {
    const colors = ['#ff6b9d', '#a18cd1', '#4ecdc4', '#ff9a9e', '#fecfef', '#fbc2eb'];
    const emojis = ['🦜', '🐦', '🕊️', '🌸', '✨', '⭐'];
...
function init() {
    ...
    document.querySelectorAll('.bird-checkbox, .formula-option, .calculate-btn').forEach(element => {
    ...
    calculateBtn.addEventListener('click', function() {
        ...
        calculateBtn.innerHTML = '<span class="btn-icon">⏳</span><span>计算中...</span>';
        ...
        setTimeout(() => {
            ...
            calculateBtn.innerHTML = '<span class="btn-icon">🧮</span><span>计算配方</span>';
        }, 800);
    });
